...
Do not reinvent the wheel. Search `src/utils/` and `third_party/` first, then reuse established parsing, hooking, hashing, archive, and memory components.
...
- `python tests/run_host_tests.py` compiles host tests with `g++` and validates JSON, hashing, ZIP safety, runtime config, and chart importing. It expects sample packages in the sibling `ArcCreate` repository.
