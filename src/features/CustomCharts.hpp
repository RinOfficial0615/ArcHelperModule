...
public:
    ...
    bool Enabled() const;
    bool IsReady() const { return hooks_installed_; }
...
private:
    ...
    AH_CFG(default_background, "base_conflict",
           [](std::string_view value) { return !value.empty(); });
    AH_CFG(default_preview_start_ms, int64_t{0}, int64_t{0}, int64_t{2147453647});
    AH_CFG(default_preview_duration_ms, int64_t{30000},
           [this](const int64_t &value) {
               return value > 0 && value <= int64_t{2147483647} - default_preview_start_ms_;
           });
